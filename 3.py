...
def lab3(n):
            ...
            for j in range(n):
                ...
                if y >= 0:
                    ...
                    try:
                        ...
                        p = lmd[l] ** y / math.factorial(y) * pow(
                            math.e,
                            -lmd[l])
                        Pr0 = p * R
    ...
    k = 10000  # Количество слотов

    # Значения лямбды
    l = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55,
         0.6, 0.65]
    ...
    ax.plot(lmd, M, marker='o',
            label='С помощью решений систем уравнений')
    ax.plot(l, N_mean, marker='o',
            label='Через количество АБ в слоте k')
    ...
    ax.set_title("При количестве состояний равном #i" # n)
    plt.legend(loc=0)
    # Ограничение графика
    # ax.set_ylim(0, 20)
